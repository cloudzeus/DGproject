 ...
 *
 * Σβήνονται ΜΟΝΟ τα προσχέδια που έφτιαξε το μοντέλο. Ό,τι μετατράπηκε σε
 * εργασία, ό,τι απορρίφθηκε ρητά, και ό,τι πρόσθεσε ο χρήστης με το χέρι
 * επιβιώνει μιας επανάληψης της ανάλυσης — αλλιώς το κουμπί «νέα προσπάθεια»
 * θα έσβηνε τη δουλειά του χρήστη.
 */
export async function persistItems(analysisId: string, items: ExtractedItem[]): Promise<void> {
  await prisma.$transaction(async (tx) => {
    await tx.proposalItem.deleteMany({
      where: { analysisId, status: 'draft', manual: false },
    })
