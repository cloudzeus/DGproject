...
Κάθε γραμμή δείχνει τα βασικά σε chips και ανοίγει **πλήρη φόρμα σε modal** με
το κουμπί επεξεργασίας ή κλικ στον τίτλο: είδος, τίτλος, περιγραφή, ημερομηνία,
ώρες, προτεραιότητα, ορατότητα, ανάδοχος, κατηγορία απαίτησης. Το ίδιο modal
δημιουργεί και νέα αντικείμενα.

> **Αναθεώρηση 2026-07-31.** Η επεξεργασία ήταν αρχικά επιτόπου στη γραμμή —
> συμπαγής, αλλά **αόρατη**: διάφανα πεδία που αποκαλύπτονταν μόνο στο focus
> μοιάζουν με σκέτο κείμενο, οπότε ο χρήστης δεν ήξερε ότι μπορεί να επέμβει.
> Και δύο πράγματα δεν χωρούσαν εκεί με τίποτα: η **αλλαγή είδους** (βήμα ↔
> ορόσημο ↔ απαίτηση) και τα πεδία των απαιτήσεων, που ήταν στην πράξη
> κλειδωμένες. Το modal είναι και συνεπές με τον κανόνα «κάθε φόρμα σε modal».

Στη γραμμή μένουν οι ενέργειες που δεν είναι φόρμα: επιλογή, απόρριψη, άνοιγμα
του `sourceQuote`, διευκρινίσεις. Χαμηλή βεβαιότητα → πορτοκαλί σήμανση, στο
μοτίβο του `meetingNeedsReview`.

Το κουμπί **«Δημιουργία εργασιών»** δουλεύει μόνο στα επιλεγμένα — σταδιακά,
όσες φορές χρειαστεί.

## Διευκρινίσεις και επαναδημιουργία

> **Προσθήκη 2026-07-30.** Η ανάθεση ήταν αρχικά εκτός σκοπού («ανάθεσε από το
> board όπου φαίνεται ο φόρτος») — αναιρέθηκε κατόπιν αιτήματος.

Η διόρθωση με το χέρι αρκεί όταν το λάθος είναι μια λέξη. Δεν αρκεί όταν είναι
δομικό: «αυτό δεν είναι ένα βήμα, είναι τρία», ή «εδώ εννοεί τον δικό μας
εξοπλισμό, όχι του πελάτη». Το μοντέλο διάβασε την πρόταση χωρίς να ξέρει
τίποτα από όσα ξέρει ο υπεύθυνος του έργου.

Κάθε γραμμή έχει «διευκρινίσεις»: γράφεις τι δεν κατάλαβε και ξαναφτιάχνεται.
Η επαναδημιουργία επιστρέφει **ένα ή περισσότερα** αντικείμενα — η συνηθέστερη
διευκρίνιση είναι ακριβώς ότι πρόκειται για πολλά.

**Η διευκρίνιση υπερισχύει της πρότασης** στο prompt. Ό,τι προκύπτει μόνο από
αυτήν παίρνει το κείμενό της ως `sourceQuote`, ώστε ο χρήστης να βλέπει αργότερα
αν μια γραμμή ήρθε από το έγγραφο ή από τον ίδιο.

Το αρχικό **δεν σβήνεται**: μένει σε `status: replaced` με τη διευκρίνιση πάνω
του. Τα παιδιά κρατούν `regeneratedFromId`, την ανάθεση και την ορατότητα του
γονέα — αυτά τα έβαλε άνθρωπος, δεν τα πρότεινε το μοντέλο.

Η διευκρίνιση **μασκάρεται κι αυτή** πριν φύγει: μπορεί κάλλιστα να γράφει
«μίλα με τον Γιώργο στο 697…».

### Επίδραση στη νέα ανάλυση

Το `persistItems` σβήνει πλέον μόνο ό,τι έφτιαξε το μοντέλο **μόνο του**:
`status='draft' AND manual=false AND regeneratedFromId IS NULL`. Χωρίς τον
τελευταίο όρο, το κουμπί «νέα ανάλυση» θα έσβηνε την ανθρώπινη γνώση και θα
ξαναέφερνε ακριβώς το βήμα που μόλις διορθώθηκε.

## Ανάθεση

Ένας ανάδοχος ανά αντικείμενο, από **όλη την ομάδα** — όχι μόνο από τα μέλη του
έργου. Η λίστα ομαδοποιείται σε «Μέλη του έργου» και «Υπόλοιπη ομάδα». Στη
μετατροπή γίνεται `TaskAssignee`.

**Ανάθεση εκτός έργου προσθέτει τον ανάδοχο ως μέλος.** Δεν είναι ευκολία: ένας
απλός χρήστης που δεν είναι `ProjectMember` δεν βλέπει καν το έργο, οπότε χωρίς
αυτό θα του ανατίθετο εργασία που δεν μπορεί να ανοίξει. Το modal το δηλώνει
πριν γίνει.

**Μία συγκεντρωτική ειδοποίηση ανά άτομο**, όχι μία ανά εργασία: η μαζική
μετατροπή είναι μία πράξη του χρήστη, και έξι χτυπήματα στο κουδουνάκι κάποιου
για ένα κλικ δεν είναι πληροφορία — είναι θόρυβος που κάνει τον επόμενο να τα
αγνοήσει όλα. Στέλνεται **εκτός** της συναλλαγής: μια αποτυχία στις
ειδοποιήσεις δεν πρέπει να γυρίσει πίσω είκοσι εργασίες που γράφτηκαν σωστά.
